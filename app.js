...
const currentTimeString=()=>new Date().toLocaleTimeString("en-GB",{timeZone:"Asia/Tokyo",hour:"2-digit",minute:"2-digit"});
const numberOrNull=v=>v===""?null:Number(v);
const STOOL_STATES={normal:"普通便",hard:"硬い便",soft:"軟便",mud:"泥状便",watery:"水様便"},LOOSE_STATES=["mud","watery"];
const hasPain=r=>r.painPresent??(Number(r.pain)>0);
const stoolStateLabel=state=>STOOL_STATES[state]??state??"";
...
function getStoolEvents(){return $$("#stoolEventList .stool-event").map(row=>({time:row.querySelector("input[type='time']").value,state:row.querySelector("select").value})).filter(e=>e.time).sort((a,b)=>a.time.localeCompare(b.time))}
function addStoolEventRow(time="",state="normal"){const row=document.createElement("div");row.className="stool-event";row.innerHTML=`<input type="time" aria-label="排便した時刻"><select aria-label="便の状態">${Object.entries(STOOL_STATES).map(([value,label])=>`<option value="${value}">${label}</option>`).join("")}</select><button type="button" class="remove-stool" aria-label="この排便記録を削除">×</button>`;row.querySelector("input").value=time;row.querySelector("select").value=state in STOOL_STATES?state:"normal";$("#stoolEventList").append(row)}
function getFormData(){const stoolEvents=getStoolEvents();return{date:$("#recordDate").value,stoolEvents,stoolCount:stoolEvents.length,looseCount:stoolEvents.filter(e=>LOOSE_STATES.includes(e.state)).length,blood:Number($("input[name='blood']:checked").value),urgency:Number($("#urgency").value),nocturnal:$("#nocturnal").checked,temperature:numberOrNull($("#temperature").value),pulse:numberOrNull($("#pulse").value),painPresent:$("input[name='pain']:checked").value==="1",weight:numberOrNull($("#weight").value),morningMeds:$("#morningMeds").checked,suppository:$("#suppository").checked,doctorMemo:$("#doctorMemo").value.trim(),personalMemo:$("#personalMemo").value.trim(),redFlags:$$('[data-red-flag]:checked').map(e=>e.value),updatedAt:new Date().toISOString()}}
...
function assessSeverity(r){
  ...
  const fulminant=severe&&r.stoolCount>=15&&r.temperature!=null&&r.temperature>=38.5&&hasPain(r);
  ...
  const incomplete=[r.temperature,r.pulse].some(v=>v==null);
  return{level:"mild",title:"入力範囲では軽症寄り",chip:incomplete?"未入力あり":"軽症の目安",summary:incomplete?"入力された症状は軽症側です。体温・脈拍が未入力のため参考の目安です。":"入力された症状は軽症側です。採血などの検査値は診察で確認し、症状が続く場合や不安がある場合は主治医へ相談してください。",flags}
}

function updateLiveResult(){const data=getFormData(),r=assessSeverity(data),card=$("#resultCard");$("#stoolLogNote").textContent=`今日の排便：${data.stoolCount}回${data.looseCount?`（うち水様・泥状 ${data.looseCount}回）`:""}`;card.classList.toggle("warning",r.level==="moderate");card.classList.toggle("danger",["severe","emergency"].includes(r.level));$("#resultTitle").textContent=r.title;$("#resultSummary").textContent=r.summary;$("#severityChip").textContent=r.chip;const activeLevel=r.level==="emergency"?"severe":r.level;$$('[data-criterion-level]').forEach(el=>el.classList.toggle("is-current",el.dataset.criterionLevel===activeLevel))}
function symptomScore(r){const base=Number(settings.baselineStools||0);return Math.max(0,r.stoolCount-base)*1.5+r.blood*3+r.urgency*1.3+(hasPain(r)?1.5:0)+(r.nocturnal?2:0)+(r.temperature>=37.5?2:0)}
...
function renderHistory(){
  ...
  const medMark=v=>v==null?"−":v?"✓":"未";
  list.innerHTML=[...records].sort((a,b)=>b.date.localeCompare(a.date)).map(r=>{const result=assessSeverity(r),d=formatDate(r.date),blood=["なし","わずか","明らか","多量"][r.blood],label=result.level==="mild"?"軽症寄り":result.level==="moderate"?"中等症項目":result.level==="severe"?"重症可能性":"緊急サイン",doctorMemo=r.doctorMemo??r.notes??"",personalMemo=r.personalMemo??"",stoolLog=(r.stoolEvents??[]).map(e=>`${e.time} ${stoolStateLabel(e.state)}`).join("、"),hasDetails=doctorMemo||personalMemo||stoolLog;return`<article class="history-item"><div class="history-date">${d.main}<small>${d.sub}</small></div><div class="history-metrics"><span>便 <b>${r.stoolCount}回</b></span><span>血便 <b>${blood}</b></span><span>腹痛 <b>${hasPain(r)?"あり":"なし"}</b></span><span>朝薬 <b>${medMark(r.morningMeds)}</b></span><span>坐薬 <b>${medMark(r.suppository)}</b></span></div><span class="severity-label ${result.level}">${label}</span><button class="delete-record" data-delete="${r.date}" aria-label="${r.date}の記録を削除">削除</button>${hasDetails?`<details class="memo-details"><summary>詳細を見る</summary>${stoolLog?`<div><b>排便の記録</b><p>${escapeHtml(stoolLog)}</p></div>`:""}${doctorMemo?`<div><b>医師に伝えるメモ</b><p>${escapeHtml(doctorMemo)}</p></div>`:""}${personalMemo?`<div><b>自分用メモ</b><p>${escapeHtml(personalMemo)}</p></div>`:""}</details>`:""}</article>`}).join("");
...
function exportCsv(){
  ...
  const medText=v=>v==null?"":v?"済":"未";
  const header=["日付","排便回数","排便の記録","血便0-3","便意切迫0-3","夜間排便","体温","脈拍","腹痛","体重","朝の内服","坐薬","医師に伝えるメモ","自分用メモ"],rows=[...records].sort((a,b)=>a.date.localeCompare(b.date)).map(r=>[r.date,r.stoolCount,(r.stoolEvents??[]).map(e=>`${e.time} ${stoolStateLabel(e.state)}`).join(" / "),r.blood,r.urgency,r.nocturnal?"あり":"なし",r.temperature??"",r.pulse??"",hasPain(r)?"あり":"なし",r.weight??"",medText(r.morningMeds),medText(r.suppository),r.doctorMemo??r.notes??"",r.personalMemo??""]),esc=v=>`"${String(v).replaceAll('"','""')}"`,csv="\uFEFF"+[header,...rows].map(row=>row.map(esc).join(",")).join("\r\n"),url=URL.createObjectURL(new Blob([csv],{type:"text/csv;charset=utf-8"})),a=document.createElement("a");a.href=url;a.download=`UC症状記録_${todayString()}.csv`;a.click();URL.revokeObjectURL(url)
}

function loadRecordToForm(date){
  const r=records.find(i=>i.date===date);$("#dailyForm").reset();$("#recordDate").value=date;$("#stoolEventList").innerHTML="";
  if(!r){updateLiveResult();return}
  (r.stoolEvents??[]).forEach(e=>addStoolEventRow(e.time,e.state));
  ["urgency","temperature","pulse","weight"].forEach(k=>{if(r[k]!==null&&r[k]!==undefined)$(`#${k}`).value=r[k]});$("#doctorMemo").value=r.doctorMemo??r.notes??"";$("#personalMemo").value=r.personalMemo??"";
  $(`input[name="blood"][value="${r.blood}"]`).checked=true;$(`input[name="pain"][value="${hasPain(r)?1:0}"]`).checked=true;$("#nocturnal").checked=r.nocturnal;$("#morningMeds").checked=!!(r.morningMeds??r.medicationTaken);$("#suppository").checked=!!r.suppository;$$('[data-red-flag]').forEach(i=>i.checked=r.redFlags?.includes(i.value));updateLiveResult()
}
...
function init(){
  ...
  $("#dailyForm").addEventListener("submit",saveRecord);$("#dailyForm").addEventListener("input",updateLiveResult);$("#addStoolEvent").addEventListener("click",()=>{addStoolEventRow(currentTimeString());updateLiveResult()});$("#stoolEventList").addEventListener("click",e=>{const button=e.target.closest(".remove-stool");if(!button)return;button.closest(".stool-event").remove();updateLiveResult()});$("#recordDate").addEventListener("change",e=>loadRecordToForm(e.target.value));$("#exportCsv").addEventListener("click",exportCsv);$("#openSettings").addEventListener("click",()=>$("#settingsDialog").showModal());
